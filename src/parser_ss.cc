...
int parse_ss(int *sN_ad, int **source_array, char *aName)
...
#define PROBLEM_VAR "ss"         /* single-source */
  ...
  int    n;                      /* internal number of sources */
  int    k;
  int   *sources=NULL;           /* internal pointer to sources */
  int source;
  char prA_type[4], pr_type[3], pr_var[3], in_line[MAXLINE];
  int no_lines= 0, no_plines=0, no_slines=0;
  FILE *aFile;
...
while (fgets(in_line, MAXLINE, aFile) != NULL)
  ...
  switch (in_line[0])
	...
	{goto error; }
      ...
      sources  = (int *) calloc(n+1, sizeof(long));
